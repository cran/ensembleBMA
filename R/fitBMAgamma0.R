...
function (ensembleData, control = controlBMAgamma0(), exchangeable = NULL) 
{
    ...
    logisticFunc <- function(x, y) {
        ...
        if (!(coefs[2] <= 0 && coefs[3] >= 0)) {
            ...
            pred <- model.matrix(fit$formula, data = fit$model)
            fit$fitted.values <- sapply(pred %*% fit$coefficients, 
                inverseLogit)
        }
        fit
    }
    ...
    completeDataLLmiss <- function(z, w, m, p0, p1, X, obs, Y0) {
        objective <- function(par) {
            ...
            -sum(z[!Y0, ][!miss] * (q[!Y0, ][!miss] + log(p1[!miss] * 
                W[!miss])))
        }
        objective
    }
    gammaLoglikNotEM <- function(w, m, p1, X, Y) {
        objective <- function(par) {
            v <- par[1]^2 + (par[2]^2) * X
            g <- array(0, dim(v))
            rate <- m/v
    ...
    gammaLoglikNotEMmiss <- function(w, m, p1, X, Y) {
        objective <- function(par) {
            nObs <- length(Y)
            nFor <- ncol(X)
            G <- X
            ...
            G[!miss] <- dgamma(matrix(Y, nObs, nFor)[!miss], 
                shape = (rate * m)[!miss], rate = rate[!miss], 
                log = TRUE)
            gmax <- apply(G, 1, max, na.rm = TRUE)
            G <- p1 * exp(G - gmax)
            G <- G * W
            -sum(gmax + log(apply(G, 1, sum, na.rm = TRUE)), 
                na.rm = TRUE)
        }
        objective
    }
    varCoefs <- if (is.null(control$start$varCoefs)) 
        c(1, 1)
    else control$start$varCoefs
    varCoefs <- pmax(varCoefs, 1e-04)
    names(varCoefs) <- names(weights) <- NULL
    weights <- if (is.null(control$start$weights)) 
        1
    else control$start$weights
    if (length(weights) == 1) 
